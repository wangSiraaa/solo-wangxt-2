...
R=$(curl -s -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
  '{"pool_id":1,"department_id":1,"employee":"张三","mode":"online","idempotency_key":"demo-online-001"}')
...
R=$(curl -s -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
  '{"pool_id":2,"department_id":2,"employee":"李四","mode":"offline","ttl_seconds":120,"idempotency_key":"demo-offline-001"}')
...
R=$(curl -s -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
  '{"pool_id":3,"department_id":3,"employee":"王五","mode":"offline","ttl_seconds":2,"idempotency_key":"demo-offline-002"}')
...
for i in $(seq 1 8); do
  (curl -s -o /dev/null -w "%{http_code}\n" -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
    "{\"pool_id\":2,\"department_id\":1,\"employee\":\"并发$i\",\"mode\":\"online\",\"idempotency_key\":\"demo-race-$i\"}" \
    > $OUT.$i) & PIDS+=($!)
done
wait "${PIDS[@]}"
...
curl -s -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
  '{"pool_id":1,"department_id":2,"employee":"孙七","mode":"online","idempotency_key":"demo-quota-001"}' > /dev/null
curl -s -X POST "$BASE/borrows" -H 'Content-Type: application/json' -d \
  '{"pool_id":1,"department_id":2,"employee":"周八","mode":"online","idempotency_key":"demo-quota-002"}' > /dev/null
...
curl -s -o /dev/null -w "超额后新申请 -> HTTP %{http_code}\n" -X POST "$BASE/borrows" \
  ...
  '{"pool_id":1,"department_id":2,"employee":"吴九","mode":"online","idempotency_key":"demo-quota-003"}'
echo "-- 恢复现场:归还并调回额度"
for k in demo-quota-001 demo-quota-002; do
